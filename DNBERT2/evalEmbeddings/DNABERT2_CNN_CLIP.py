...
output_dir = "/gpfs/bwfor/work/ws/fr_ml642-thesis_work/Thesis/DNBERT2/evalEmbeddings/results/results_clip_data_uhl"
# Use workspace to avoid home directory quota limits
model_save_dir = "/gpfs/bwfor/work/ws/fr_ml642-thesis_work/Thesis/DNBERT2/models/models_clip_data_uhl"
    ...
    for split_name, (seqs, labels) in splits.items():
        ...
        with torch.no_grad():
            for i in tqdm(range(0, len(seqs), extraction_batch_size), desc=f"Embed {split_name}"):
                ...
                hidden_states = dnabert2_model(inputs)[0]  # [batch, seq_len, 768]
                # Mean pooling over sequence length
                embeddings = torch.mean(hidden_states, dim=1)  # [batch, 768]
