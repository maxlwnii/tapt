"""
linear_probe_cross_cell.py
==========================
Unified linear probing evaluation for ALL model variants using the
cross-cell generalisation dataset (Thesis/data/cross_cell/).
...
Usage
-----
    python linear_probe_cross_cell.py                          # uses DNABERT-2 layer 4 / LAMAR layer 6 by default
    python linear_probe_cross_cell.py --models lamar_pretrained dnabert2_pretrained
    python linear_probe_cross_cell.py --best_layer_override 9
    python linear_probe_cross_cell.py --enable_layer_search --max_rbps 5
"""
...
_SCRIPT_DIR  = Path(__file__).resolve().parent          # …/linear_probe_cross_cell/
...
_DEFAULT_DATA_ROOT = f"{_BASE}/data/cross_cell"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Linear probing on cross-cell data for all DNA-LM variants"
    )
    ...
    p.add_argument("--max_rbps",               type=int, default=0,
                   help="If >0, evaluate only first N tasks (useful for smoke tests)")
    ...
    p.add_argument(
        "--output_dir",
        type=str,
        default=f"{_BASE}/linear_probe_cross_cell/results",
    )
    p.add_argument(
        "--cache_dir",
        type=str,
        default=f"{_BASE}/linear_probe_cross_cell/results/cache",
    )
...
def load_tasks(data_root: str) -> Dict[str, Dict[str, pd.DataFrame]]:
    ...
    for task_dir in sorted(p for p in root.iterdir() if p.is_dir()):
        ...
        # FIX: preserve cross-cell protocol by keeping train+dev and test separated.
...
def evaluate_linear_probe(
    ...
    # FIX: fit probe on train+dev and evaluate once on held-out test to preserve cross-cell split.
    clf = make_pipeline(
        StandardScaler(),
        LogisticRegression(max_iter=5000, C=1.0, random_state=seed),
    )
...
def plot_boxplot(df: pd.DataFrame, out_path: Path) -> None:
    ...
    plt.ylabel("AUROC")
    plt.title("AUROC distribution across cross-cell tasks")
...
def plot_per_rbp_bars(df: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.set_ylabel("AUROC")
    ax.set_title("Per-task AUROC by model — cross-cell generalisation")
...
def plot_delta(df: pd.DataFrame, out_path: Path) -> None:
    ...
    plt.title("Per-task AUROC delta — cross-cell generalisation")
    plt.xticks(rotation=70, ha="right", fontsize=6)
    plt.tight_layout()
    plt.savefig(out_path, dpi=200)
    plt.close()
...
def plot_embedding_projections(
    ...
    for model_name, (X, task_idx) in embeddings_by_model.items():
        ...
        plt.title(f"{model_name}: {method.upper()} — cross-cell tasks")
        plt.xlabel("dim-1")
        plt.ylabel("dim-2")
        if len(uniq) <= 15:
