...
class SaliencyAnalyzer:
    def __init__(self, model_path, device=None):
        ...
        self.tokenizer_path = "/home/fr/fr_fr/fr_ml642/Thesis/pretrain/saving_model/tapt_lamar/checkpoint-100000/"
        self.model = None   
        self.tokenizer = None
        self.load_model()
    ...
    def load_model(self):
        ...
        try:
            self.tokenizer = EsmTokenizer.from_pretrained(self.tokenizer_path)
    ...
    def _encode(self, seq, with_offsets=False):
        if with_offsets and not self.tokenizer.is_fast:
            # For non-fast tokenizers, don't use return_offsets_mapping
    ...
    def extract_offset_mapping(self, sequence):
        ...
        if 'offset_mapping' in inputs:
            ...
            # Approximate for non-fast tokenizers
            all_tokens = self.tokenizer.convert_ids_to_tokens(inputs['input_ids'][0])
            token_ids = inputs['input_ids'][0].tolist()
            L = len(sequence)
            # Assume tokens are [CLS] sequence_tokens [SEP], skip specials
            filtered_tokens = all_tokens[1:-1] if len(all_tokens) > 2 else all_tokens
            filtered_token_ids = token_ids[1:-1] if len(token_ids) > 2 else token_ids
            num_tokens = len(filtered_tokens)
            if num_tokens == 0:
                filtered_offset_mapping = []
            else:
                token_length = L // num_tokens
                filtered_offset_mapping = []
                current_pos = 0
                for i in range(num_tokens):
                    start = current_pos
                    end = current_pos + token_length if i < num_tokens - 1 else L
                    filtered_offset_mapping.append((start, end))
                    current_pos = end
    ...
    def compute_gradient_saliency(self, sequence, target_class=None):
        ...
        if self.tokenizer.is_fast:
            ...
            # For non-fast tokenizers, assume token_scores[1:-1] correspond to sequence tokens
            token_scores_kept = token_scores[1:-1]
    ...
    def compute_integrated_gradients_saliency(self, sequence, target_class=None, n_steps=120):
       ...
       # pad_id = self.tokenizer.pad_token_id  ---> leads to considerable worse performance.
        ...
        enc_offsets = self._encode(sequence, with_offsets=True)
        if 'offset_mapping' in enc_offsets:
            keep_mask = [not (s == 0 and e == 0) for (s, e) in enc_offsets['offset_mapping'][0].tolist()]
            token_attr_kept = np.array([ts for ts, keep in zip(token_attr, keep_mask) if keep])
        else:
            # For non-fast tokenizers
            token_attr_kept = token_attr[1:-1]
...
def get_user_input():
    print("=== RNA Sequence Saliency Analysis Tool ===")
    model_path = input("Enter the path to your trained model directory [/gpfs/bwfor/work/ws/fr_ml642-thesis_work/Thesis/LAMAR/models/finetuned_limited/TAPT/PTBP1_HepG2_IDR/fold_5/checkpoint-240]: ").strip()
    if not model_path:
        model_path = "/gpfs/bwfor/work/ws/fr_ml642-thesis_work/Thesis/LAMAR/models/finetuned_limited/TAPT/PTBP1_HepG2_IDR/fold_5/checkpoint-240"

    print("\nChoose sequence input method:")
    print("Enter sequence manually")

    sequence = input("Enter RNA sequence (A/T/G/C/U only): ").strip().upper().replace("U", "T")
